...
``PUBLIC_AI_ENABLED=false``, ``PUBLIC_TTS_ENGINE=browser``. The root layout
(``src/routes/+layout.ts``) sets ``prerender = true`` (required when using a static
``fallback``, or pages without their own prerender flag are omitted from the build —
which made GitHub Pages serve ``404.html`` for ``/``) and ``trailingSlash = 'always'``
so project Pages URLs like ``/<repo>/`` resolve on first load. Override any knob with
...
``PUBLIC_AI_ENABLED``, ``PUBLIC_AI_BASE_URL``. Model, embed model, tutor name/prompt,
API mode, and TTS voice come from ``GET /v1/soju/config/client`` when a backend is
reachable (see :doc:`ai`); otherwise ``apps/web/src/lib/config.ts`` fallbacks apply.
