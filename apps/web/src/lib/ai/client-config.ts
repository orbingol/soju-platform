...
/**
 * Fetch ``GET /v1/soju/config/client`` once and apply model/tutor/TTS defaults.
...
export function ensureClientConfig(): Promise<boolean> {
  if (!loadPromise) {
    loadPromise = (async () => {
      try {
        const response = await fetchWithTimeout(apiUrl('/v1/soju/config/client'), { method: 'GET' }, AI_FETCH_TIMEOUT_MS);
