...
describe('sanitizeHtml', () => {
  ...
  it('keeps allowlisted markup used in topic descriptions', () => {
    const html = '<p>Use <strong>위</strong> and <em>에</em>. <br> Also <b>bold</b> <i>italic</i>.</p>';
    expect(sanitizeHtml(html)).toBe('<p>Use <strong>위</strong> and <em>에</em>. <br> Also <b>bold</b> <i>italic</i>.</p>');
  });

  it('allows safe http(s) and hash hrefs on anchors', () => {
    expect(sanitizeHtml('<a href="https://example.com/x">link</a>')).toBe('<a href="https://example.com/x">link</a>');
  ...
  it('strips unsafe hrefs and non-href attributes', () => {
    ...
    expect(sanitizeHtml('<a href="https://ok.test" onclick="evil()" class="x">x</a>')).toBe('<a href="https://ok.test">x</a>');
  });
