...
"""List and assign course levels on vocabulary registry entries."""
...
def list_unassigned(root: Path | None = None, *, type_id: str | None = None) -> list[dict[str, Any]]:
    """Return vocabulary entries with no ``level`` (optionally filtered by type)."""
...
def set_levels(
    ...
    *,
    ...
    """Set ``level`` on selected vocabulary entries.
    ...
    if all_unassigned:
        ...
        if already:
            raise ValueError(
                "Already tagged (pass --force to overwrite): " + ", ".join(already)
            )
    ...
    return SetLevelsReport(updated=len(targets), level_id=resolved, dry_run=dry_run)


def parse_ids_file(text: str) -> list[str]:
    """Parse UUID lines from an ids file (blank lines and ``#`` comments ignored)."""
    ids: list[str] = []
