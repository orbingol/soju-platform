...
"""Run check-jsonschema over all canonical data files (no CLI)."""
...
def validate_schemas(root: Path | None = None) -> list[str]:
    ...
    for name, files in targets.items():
        ...
        try:
            ...
            errors.append(f"{name}: check-jsonschema is not installed or not on PATH (install project deps with `uv sync`).")
            break
