...
"""Validate verb forms and examples against table layout and vocabulary (no CLI)."""

from __future__ import annotations

from soju.registry.examples import load_examples_store
from soju.registry.verbs import iter_verb_columns, load_verb_forms_file
...
def validate_alignment(table: dict, verbs: list[dict], root=None) -> list[str]:
    ...
    forms_by_tense = {tense: load_verb_forms_file(tense, root) for tense in tense_ids}

    for tense_id, column in iter_verb_columns(table):
